...
Isso acontece porque muitos aposentandos começaram a trabalhar antes do plano Real em 1994 e seus salários anteriores em cruzeiros
 muitas vezes são relevantes para o cálculo da sua aposentadoria hoje.

Nesta questão você vai desenvolver uma calculadora com as 4 operações básicas. Para simplificar um pouco a questão 
vamos utilizar a notação polonesa, que funciona assim:

+ 1 2

No exemplo acima nós temos a operação ( 1 + 2 ) que retorna o valor 3. Vamos chamar esse caso de expressão simples,
 pois tem apenas 1 operador (no caso adição). Essa notação foge um pouco dos padrões que aprendemos na escola, mas tem uma vantagem bem interessante: 
 ela dispensa o uso de parênteses. Veja só como ficaria a operação ( 1 + 18 / ( 2 * 3 ) ):
...
Perfeito, temos a mesma operação de antes. E sem precisar de parênteses fica mais fácil de implementar. Agora vamos fazer um tributo à linguagem de 
computador Lisp e separar cada operação e sub-operação em uma lista encadeada:

[ + 1 [ / 18 [ * 2 3 ] ] ]

Você deverá implementar a função calculate no código da aba ao lado para lidar com os problemas listados acima. As assinaturas da entrada e da saída
 da questão já estão montadas para você.

Por último, como podemos ter erro na divisão por zero vamos ter que lidar com isso de alguma forma. Para simplificar a implementação vamos 
retornar null esses casos de erro.
